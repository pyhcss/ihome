    ...
    def post(self):
        try:
            ...
            if not all((mobile,password)):
                ...
                try:
                    user = self.db.get("select ui_id,ui_title,ui_tel,ui_pwd,ui_image from user_info where ui_tel=%s",mobile)
                except Exception as e:
                    print e
                    ...
                    if not user:
                        ...
                        try:  # 调用session接口设置session_id
                            ...
                            if user["ui_image"]:
                                self.session.data["avatar"] = IMAGE_URL_START + user["ui_image"]
                            self.session.save()
                        except Exception as e:
                            print e  # 出现异常返回错误信息
                            return self.write({"errcode": "4102", "errmsg": "登录失败"})
